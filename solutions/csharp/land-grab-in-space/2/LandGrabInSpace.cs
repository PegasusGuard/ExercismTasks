...
public struct Plot
{
    ...
    public uint MaxSide() 
    {
        ...
        {
            (ushort)(Math.Pow(_rightTop.X-_leftTop.X, 2) + Math.Pow(_rightTop.Y-_leftTop.Y, 2)),
            (ushort)(Math.Pow(_leftTop.X-_leftBottom.X, 2) + Math.Pow(_leftTop.Y-_leftBottom.Y, 2)),
            (ushort)(Math.Pow(_leftBottom.X-_rightBottom.X, 2) + Math.Pow(_leftBottom.Y-_rightBottom.Y, 2)),
            (ushort)(Math.Pow(_rightBottom.X-_rightTop.X, 2) + Math.Pow(_rightBottom.Y-_rightTop.Y, 2))
        };
        return sides.Max();
    }
}
